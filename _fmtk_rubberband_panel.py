...
class RubberbandPanel(wx.ScrolledWindow):
    ...
    def __init__(self, a_frame, an_app, any_id):
        wx.ScrolledWindow.__init__(self, a_frame, any_id, wx.DefaultPosition,
                                   ...
                                   wx.HSCROLL | wx.VSCROLL, 'ID_LEAFPANEL')
        self.frame = a_frame
        self.app = an_app
    ...
    def load_image(self):
        if self.standalone:
            ...
            self.szd_image = Image.open('softalkv2n07mar1982_0004.jpg')
        else:
            self.szd_image = self.src_image

        # First, do the full-page image scaled to ML model image max_pixels...
        self.resize_dimensions = self.cap_dimensions(self.szd_image.size[0],
                                                     self.szd_image.size[1])
        # Resize the szd_image to the constraint of the maximum number of
        ...
        self.szd_image = self.szd_image.resize(self.resize_dimensions,
                                               Image.Resampling.BICUBIC)
        self.szd_img2buffer = wx.Image(self.szd_image.size[0],
                                       self.szd_image.size[1])
        self.szd_img2buffer.SetData(self.szd_image.convert("RGB").tobytes())
        try:
            self.buffer = self.szd_img2buffer.ConvertToBitmap()
        except KeyError:
    ...
    def unscale_dimensions(self, width, height):
        return wx.Size(int(width * self.img_scale),
                       int(height * self.img_scale))
    # end unscale_dimensions

    def unscale_position(self, x, y):
        return wx.Point(int(x * self.img_scale), int(y * self.img_scale))
    # end unscale_position

    def unscale_point(self, point):
        return wx.Point(int(point.x * self.img_scale), int(point.y * self.img_scale))
    # end unscale_position

    def unscale_bbox(self, bbox):
        return wx.Rect(int(bbox.x * self.img_scale),
                       int(bbox.y * self.img_scale),
                       int(bbox.width * self.img_scale),
                       int(bbox.height * self.img_scale))
    # end unscale_bbox
